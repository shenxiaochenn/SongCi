...
def get_model(model="dino", use_head=False, pretrained_weights=None):
    '''
    ...
    if model == "supervised":
        ...
        #check_point = "/home/wangzhenyuan/dino_protptype/human_patch_shift_2023_08_13/checkpoint.pth"
        check_point = "/home/wangzhenyuan/pathology/multi_modality/checkpoint_1018_220.pth"
        state_dict = torch.load(check_point, map_location="cpu")
        if "teacher" in state_dict:
            print("Take key teacher in provided checkpoint dict")
            state_dict = state_dict["teacher"]
        state_dict = {k.replace("module.", ""): v for k, v in state_dict.items()}
        state_dict = {k.replace("backbone.", ""): v for k, v in state_dict.items()}
        msg = model.load_state_dict(state_dict, strict=False)
        print('Pretrained weights found at {} and loaded with msg: {}'.format(check_point, msg))
        return Wrapper(model, None, use_head)
